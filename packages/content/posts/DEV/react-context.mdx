...
function DependencyProvider({ children }) {
  ...
  return (
    <DependencyContext.Provider value={{ postViewModel }}>{children}</DependencyContext.Provider>
  );
}
...
function PostManagerProvider({ children }) {
  ...
  return <PostManagerContext.Provider value={manager}>{children}</PostManagerContext.Provider>;
}
```
