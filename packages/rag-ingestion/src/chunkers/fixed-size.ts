...
export class FixedSizeChunker implements ChunkingStrategy {
  ...
  async chunk(content: string, options?: ChunkingOptions): Promise<Chunk[]> {
    ...
    while (currentPos < content.length) {
      ...
      if (chunkText.length > 0) {
        chunks.push(
          this.createChunk(chunkIndex, chunkText, currentPos, currentPos + chunkText.length, opts)
        );
        chunkIndex++;
      }
   ...
   */
  private findNextSeparator(content: string, startPos: number, separators: string[]): number {
