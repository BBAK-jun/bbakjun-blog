...
import { usePopularPostsStats } from '@/lib/hooks/use-popular-posts'
import PopularPostSkeleton from './skeleton/PopularPostSkeleton'

interface StreamingPopularPostsGridProps {
  limit?: number
  className?: string
}

export default function StreamingPopularPostsGrid({
  limit = 12,
  className = ''
}: StreamingPopularPostsGridProps) {
  const { data: stats, isLoading, error } = usePopularPostsStats()
    ...
    return (
      <div className={`${className}`}>
        <div className="text-center p-12">
          <div className="text-red-500 dark:text-red-400 p-6 bg-red-50 dark:bg-red-900/20 rounded-lg">
            <h3 className="text-lg font-medium mb-2">데이터를 불러올 수 없습니다</h3>
            <p className="text-sm">인기 글 목록을 가져오는 중 오류가 발생했습니다.</p>
          </div>
        </div>
      </div>
    )
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-6 shadow-sm">
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              <div className="h-8 bg-gray-200 dark:bg-gray-700 rounded w-24 mb-1 animate-pulse" />
