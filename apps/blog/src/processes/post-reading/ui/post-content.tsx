...
import { usePost } from '@/features/posts/lib/use-posts';
...
export default function PostContent({ slug, initialPost, initialHtmlContent }: PostContentProps) {
  ...
  const { frontMatter, readingTime } = currentPost;
      ...
      <div className="space-y-4 mb-12">
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className="space-y-2">
            <div
              className={`h-4 bg-gray-200 dark:bg-gray-700 rounded ${i % 3 === 0 ? 'w-4/5' : 'w-full'}`}
            />
