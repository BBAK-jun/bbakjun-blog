...
export default function ViewCounter({ slug, increment = false, className = '' }: ViewCounterProps) {
  ...
  if (error) {
    return (
      <span className={`text-gray-500 dark:text-gray-400 ${className}`}>조회수 로드 실패</span>
    );
  }
...
export function ViewBadge({ slug, className = '' }: ViewBadgeProps) {
  ...
  if (loading) {
    return (
      <div
        className={`inline-flex items-center px-2 py-1 text-xs bg-gray-100 dark:bg-gray-700 rounded-full ${className}`}
      >
  ...
  return (
    <div
      className={`inline-flex items-center px-2 py-1 text-xs bg-blue-100 dark:bg-blue-900 text-blue-800 dark:text-blue-200 rounded-full ${className}`}
    >
